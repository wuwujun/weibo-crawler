...
class WeiBoScraper(object):
    ...
    def __init__(self, using_account, uuid, filter_flag=1):
        """
    ...
    def _get_html(self):
        try:
            if is_number(self.user_id):
                url = 'https://weibo.cn/u/%s?filter=%s&page=1' % (self.user_id, self.filter)
                print(url)
            else:
                url = 'https://weibo.cn/%s?filter=%s&page=1' % (self.user_id, self.filter)
                print(url)
        ...
        try:
            ...
            try:
                ...
                for page in range(1, 2):
                    url2 = 'https://weibo.cn/%s?filter=%s&page=%s' % (self.user_id, self.filter, page)
                    ...
                    if len(info) > 3:
                        for i in range(0, len(info) - 2):
                            ...
                            self.weibo_detail_urls.append('https://weibo.cn/comment/{}?uid={}&rl=0'.
                            ...
                            temp_time = str_time[0].xpath('string(.)')
                            pattern_1 = "\d\d月\d\d日 \d\d:\d\d"
                            pattern_2 = "\d{4}-\d\d-\d\d \d\d:\d\d:\d\d"
                            is_match_1 = re.match(pattern_1, temp_time)
                            if is_match_1:
                                local_year = time.localtime(time.time()).tm_year
                                temp = is_match_1.group()
                                final_time = str(local_year) + '-' + temp[0:2] + '-' + temp[3:5] + temp[6:] + ":00"
                            else:
                                is_match_2 = re.match(pattern_2, temp_time)
                                if is_match_2:
                                    temp = is_match_2.group()
                                    final_time = temp_time[:(len(temp)-3)]
                            self.weibo_time.append(final_time)
    ...
    def _save_weibo_content(self):
        db = pymysql.connect(host="localhost", user="root", password="19970103", database="mydb", charset="utf8")
        cursor = db.cursor()
        sql_query_1 = "SELECT * FROM weibo_crawler WHERE labelid = " + "'" + self.user_id + "'"
        sql_query_2 = "SELECT MAX(id) from weibo_crawler WHERE labelid = " + "'" + self.user_id + "'"
        sql_insert = "INSERT INTO weibo_crawler(id, time, text, labelid) VALUES (%s, %s, %s, %s)"
        sql_values = []
        count = len(self.weibo_content)
        ...
        try:
            ...
            if not results_1:
                for i in range(1, count + 1):
                    temp = []
                    temp.append(i)
                    temp.append(self.weibo_time[i-1])
                    temp.append(self.weibo_content[i-1])
                    temp.append(self.user_id)
                    sql_values.append(temp)
            else:
                try:
